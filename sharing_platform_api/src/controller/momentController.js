...
class MomentController {
    ...
    async list(ctx) {
        ...
        const {
            count
        } = await momentService.getCountMoment();
        ctx.body = resultObj(200, "查询成功", result, count);
    }
    ...
    async increaseViews(ctx) {
        ...
        if (!momentId || !count) return console.log("momentId或者count不能为空");
        const result = await momentService.increaseViews(momentId, count);
        ctx.body = resultObj(200, "修改成功", result);
    }
    ...
    async getMomentByUserId(ctx) {
        const {
            userId
        } = ctx.params;
        const moments = await momentService.getMomentByUserId(userId);
        ctx.body = resultObj(200, "查询成功", moments, moments.length);
    }
    async getMomentByLabel(ctx) {

        const {
            id
        } = ctx.params;
        const moments = await momentService.getMomentByLabel(id);
        ctx.body = resultObj(200, "查询成功", moments, moments.length);
    }
    async getMomentByViews(ctx) {
        const {
            offset,
            limit
        } = ctx.query;
        const moments = await momentService.getMomentByViews(offset, limit);
        ctx.body = resultObj(200, "查询成功", moments);
    }
}
